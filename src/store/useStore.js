...
function initLS() {
  if (!LS.get('bk_initialized', false)) {
    ...
    LS.set('bk_chats', []);
    LS.set('bk_users', [
      { id: 'user-demo', name: '示範用戶', email: 'demo@example.com', password: 'demo123', role: 'customer' },
      { id: 'admin-001', name: '系統管理員', email: 'admin@example.com', password: 'admin123', role: 'admin' },
    ]);
    LS.set('bk_usedCoupons', {}); // { userId: [couponId, ...] }
...
const useStore = create((set, get) => ({
  ...
  // ── Auth ──────────────────────────────────────────────────
  currentUser: LS.get('bk_currentUser', null),
  login: (email, password) => {
    const users = LS.get('bk_users', []);
    const user = users.find(u => u.email === email && u.password === password);
    if (user) {
      const safeUser = { id: user.id, name: user.name, email: user.email, role: user.role };
      LS.set('bk_currentUser', safeUser);
  ...
  logout: () => {
    LS.set('bk_currentUser', null);
    set({ currentUser: null });
  },
  register: (name, email, password) => {
    const users = LS.get('bk_users', []);
    if (users.find(u => u.email === email)) return { success: false, error: '此Email已被使用' };
    const newUser = { id: `user-${Date.now()}`, name, email, password, role: 'customer' };
    users.push(newUser);
    LS.set('bk_users', users);
    const safeUser = { id: newUser.id, name, email, role: 'customer' };
    LS.set('bk_currentUser', safeUser);
    set({ currentUser: safeUser });
    return { success: true, user: safeUser };
  },
  ...
  cancelOrder: (orderId) => {
    ...
    const refundAmount = Math.round(order.finalAmount * refundPercent / 100);
  ...
  sendMessage: (fromUserId, toUserId, propertyId, message) => {
    ...
    if (existing) {
      ...
      chats.push({ key: chatKey, propertyId, participants: [fromUserId, toUserId], messages: [newMsg] });
    }
